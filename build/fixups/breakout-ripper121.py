"""breakout-ripper121 -- port from the pre-June-2014 Gamebuino API.
...
This sketch only ever calls setTextSize(1), which maps straight onto
fontSize = 1 -- the Classic library's own default -- so the text is
...
for leftover in ('setCursor', 'setTextSize', 'setTextWrap'):
    if leftover in text:
        raise RuntimeError('breakout: %s still present after port' % leftover)
